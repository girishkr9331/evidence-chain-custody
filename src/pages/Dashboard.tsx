      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-6">
          {statCards.map((stat, index) => (
            <div key={index} className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-3 md:p-6 border border-gray-200 dark:border-gray-700">
              <div className="flex items-center justify-between mb-2 md:mb-4">
                ...
                {stat.trend.show && (
                  <span className={`text-xs md:text-sm font-medium flex items-center gap-1 ${
                    ...
                    <TrendingUp className={`w-3 h-3 md:w-4 md:h-4 ${!stat.trend.isPositive ? 'rotate-180' : ''}`} />
                    <span className="hidden sm:inline">{stat.trend.percentage}</span>
                  </span>
                )}
              </div>
              <h3 className="text-gray-600 dark:text-gray-300 text-xs md:text-sm font-medium mb-1">{stat.title}</h3>
              <p className="text-xl md:text-3xl font-bold text-gray-900 dark:text-white">{stat.value}</p>
            </div>
          ))}
        </div>
